...
!      print*, 'current SF:',SUM(EMS_SF(:,:,:,:)),
!     &         MAXVAL(EMS_SF(:,:,:,:)),MINVAL(EMS_SF(:,:,:,:))
!      print*, 'current SF_ADJ:',SUM(EMS_SF_ADJ(:,:,:,:)),
!     &         MAXVAL(EMS_SF_ADJ(:,:,:,:)),MINVAL(EMS_SF_ADJ(:,:,:,:))
!      print*, 'priori SF', SUM(EMS_SF0(:,:,:,:)),
!     &         MAXVAL(EMS_SF0(:,:,:,:)),MINVAL(EMS_SF0(:,:,:,:))
!!!$OMP PARALLEL DO
!!!$OMP+DEFAULT( SHARED )
!!!$OMP+PRIVATE( I, J, M, N, REG_4D, S2_INV_2D )
      DO N = 1, NNEMS
            !print*, 'EMS error:',N,EMS_ERROR(N)
     ...
     &                 -1,     MATINVX, MATY                          )
         ...
         DO M = 1, MMSCL
            ...
            DO I = 1, IIPAR
               ...
               S2_INV_2D(I,J)= 1d0/( EMS_ERROR(N)/EXP(EMS_SF0(I,J,M,N)))
!     &                          ** 2
#else
